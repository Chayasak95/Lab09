package com.sqa.lab.service;

import static org.junit.jupiter.api.Assertions.*;
import static org.mockito.Mockito.mock;
import static org.mockito.Mockito.verify;
import static org.mockito.Mockito.when;

import java.util.Arrays;
import java.util.List;

import org.junit.jupiter.api.Test;

import com.sqa.lab.api.*;

class Favoritemoviemocktest {

	@Test
	void testFavoriteMoviesActionPlaylist_Mock() {
		MoiveService service = mock(MoiveService.class);

     
        when(service.getMovie("user1", "action")).thenReturn(Arrays.asList("F1", "Mission Impossible", "How to Train Your Dragon"));

        FavoriteMovie fav = new FavoriteMovie(service);
        List<String> movies = fav.showMovies("user1", "action");

        assertEquals(Arrays.asList("F1", "Mission Impossible", "How to Train Your Dragon"), movies);

        verify(service).getMovie("user1", "action");
    }
}
